...
class CalibrationHandler():
    ...
    def export_jump_info(self):
        ...
        directory = 'info_exports'

        # Criar o diretório se ele não existir
        if not os.path.exists(directory):
            os.makedirs(directory)

        # Salvar o arquivo no diretório criado
        plt.savefig(f'{directory}\\{name_base}_graph.png')
        ...
        for frame_num in range(0, self.frame_count):
            ...
            if ankle_check:
                ...
                if not first_ankle_frame:
                    self.ankle_averages.append(((0, 0), (0, 0)))  # Adiciona uma tupla de coordenadas de tornozelo vazias
                else:
